...
selectivity = list(model = c(rep("logistic", groundfish_info$n_fleets),rep("logistic", groundfish_info$n_indices))) #fleet, index

M = list(initial_means = rep(0.2, length(groundfish_info$ages)))

#initial numbers at age
NAA_re = list(N1_pars = rep(exp(10), length(groundfish_info$ages)))
