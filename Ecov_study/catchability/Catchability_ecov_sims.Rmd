...
for(iom in 1:nrow(OMsetup)){
  ...
  for(iem in 1:nrow(EMsetup)){
    ...
    if(EMsetup[iem, "miss_q"] == "NoEcov"){ # No ecov impact on q or q random effect
      ...
      input$model_name <- paste0("EM_", paste(unfactor(EMsetup[iem, ]), collapse = "_")) # Set up OM name based on OMsetup 
...
# Subset of OMs to run
subsetOM <- OMsetup %>% filter(F_hist == "H-L")
...
```{r}
# ID all simulation result files 
files <- list.files(path = here::here(), pattern = "simWHAM_", recursive = TRUE)

postprocess_simTestWHAM(filenames = files) 
```
