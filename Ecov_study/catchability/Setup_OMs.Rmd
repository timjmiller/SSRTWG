...
2) Pick subset of OM settings and vary seasonality of q effect in EM
```{r}
##### EM
...
for(iom in 1:nrow(OMsetup)){
  ...
  # Ecov setup
    Ecov <- list(label = "Ecov",
                 mean = matrix(rep(0,length(groundfish_info$years)), ncol =1), # Mean = 0
                 logsigma = matrix(log(OMsetup$Ecov_process_obs_sig[iom]), length(groundfish_info$years), ncol=1),
                 years = groundfish_info$years, 
                 use_obs = matrix(rep(TRUE, length(groundfish_info$years)), ncol = 1),
                 lag = 0, 
                 where = "q", # Where/how/indices settings need to change if we do sensitivity runs
                 how = 1,
                 indices = list(1,2),
                 process_model = "ar1", # End generic inputs for ecov
                 process_mean_vals = rep(0,length(groundfish_info$years)), # Mean = 0
...
#   # Fishing history
#   # input =
#           set_F_scenario(input, Fhist, Fmsy = Fmsy, max_mult = max_mult_Fmsy, min_mult= min_mult_Fmsy,
...
# Observation error - set L-N SD parameters for catch and index age comps
