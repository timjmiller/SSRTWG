...
Check that ecov effect = 0 is essentially turning off ecov = yes this is already implemented in step 1 outlined above
```{r}
# Catchability set up
...
Use unique copy of make_om() to set up generic OM input then add additional ecov settings afterwards before fitting model
...
for(iom in 1:nrow(OMsetup)){
  ...
  mkdir(here::here("Results", paste0("OM_", OMsetup$OMname[iom]))) # set up storage
  saveRDS(OM, file = here::here("Results", paste0("OM_", OMsetup$OMname[iom]), paste0("OM_", OMsetup$OMname[iom], ".Rds"))) # save OM
  saveRDS(OMsetup[iom,], file = here::here("Results", paste0("OM_", OMsetup$OMname[iom]), "OMsettings.Rds")) # save OM settings in a separate file since naming is based on arbitraty order in OMsetup table
