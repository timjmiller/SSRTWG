...
#  stop()
  ...
  if(is.null(NAA_re$recruit_model) || NAA_re$recruit_model < 3){ # If no S-R or R random effects then FMSY not estimated, use F40 instead
    F40 = exp(temp$rep$log_FXSPR[brp_year]) %>% round(., 3)
    if(is.na(F40) || F40 < 1e-5 || F40 > 10) stop("search for F40 failed, try changing eq_F_init. \n")
    
    Fmsy = F40 # Use F40% proxy as Fmsy
    
    NAA_re$N1_model = 1 # Estimate initial R and full F to determine an equilibrium initial NAA
      # Not the same process used to set up initial NAA at equilibrium conditions when S-R used (see below)
