...
plot_df_fn <- function(df.ems, df.oms, M_est=FALSE, Ecov_est = FALSE) {
  ...
  for(i in 1:3) {
    ...
    em_ind <- which(df.ems$Ecov_est == Ecov_est & df.ems$M_est == M_est & df.ems$re_config == re_mod)
    print("em_ind")
    print(em_ind)
    om_ind <- which(df.oms$NAA_M_re == re_mod) #om and em match
#    print(om_ind)
    res <- lapply(om_ind, function(x) {
      M_res <- t(sapply(1:40, function(y) {
        #print(om_ind)
        r_e <- M_bias[[x]][,em_ind,y,2]/M_bias[[x]][,em_ind,y,1]-1
        out <- c(median(r_e, na.rm = TRUE), sd(r_e, na.rm=T)/sqrt(sum(!is.na(r_e))), custom_boxplot_stat(r_e))#quantile(x, probs = c(0.025,0.975)))
        return(out)
      }))
      colnames(M_res) <- c("bias_est", "bias_se", "ymin", "lower", "middle", "upper", "ymax")
      return(M_res)
    })
    # print(res[[1]])
    # print(dim(res[[1]]))
...
plt <- ggplot(temp, aes(x = Ecov_effect, y = bias_est, colour = year)) + scale_colour_viridis_d(alpha = 0.7) + 
    ...
    ylab(bquote(Median~relative~error~of~M)) + xlab(expression(beta[Ecov])) +
...
plt
ggsave(here("Ecov_study","mortality", "paper", "annual_M_bias_OM_EM_PE_match_M_fixed_beta_fixed.png"), plt, width = 20, height = 12, units = "in")
remove(all_res)
...
plt <- ggplot(temp, aes(x = Ecov_effect, y = bias_est, colour = year)) + scale_colour_viridis_d(alpha = 0.7) + 
    ...
    geom_errorbar(aes(ymin = ymin, ymax = ymax), width = .5, position = position_dodge(0.6), linewidth = 1)
plt
ggsave(here("Ecov_study","mortality", "paper", "annual_M_bias_OM_EM_PE_match_M_fixed_beta_estimated.png"), plt, width = 20, height = 12, units = "in")
remove(all_res)
...
plt <- ggplot(temp, aes(x = Ecov_effect, y = bias_est, colour = year)) + scale_colour_viridis_d(alpha = 0.7) + 
    ...
    geom_errorbar(aes(ymin = ymin, ymax = ymax), width = .5, position = position_dodge(0.6), linewidth = 1)
plt
ggsave(here("Ecov_study","mortality", "paper", "annual_M_bias_OM_EM_PE_match_M_estimated_beta_estimated.png"), plt, width = 20, height = 12, units = "in")
remove(all_res)
