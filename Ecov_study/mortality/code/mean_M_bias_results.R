...
plot_df_fn <- function(df.ems, df.oms, Ecov_est = FALSE) {
  for(i in 1:3) {
    ...
    em_ind <- which(df.ems.$Ecov_est == Ecov_est & df.ems.$re_config == re_mod)
...
all_res <- plot_df_fn(df.ems, df.oms, Ecov_est = FALSE)
plt <- ggplot(all_res, aes(x = Ecov_effect, y = bias_est, colour = Ecov_re_sig:Ecov_re_cor)) + scale_colour_viridis_d() + 
    ...
    facet_grid(Ecov_obs_sig + obs_error ~ NAA_M_re + Fhist, labeller = labeller(obs_error = label_wrap_gen(width = 35), Ecov_obs_sig = label_parsed)) +
    theme_bw() + coord_cartesian(ylim = c(-1, 1)) + ylab(bquote(Median~Bias~of~beta[M])) + xlab(expression(beta[Ecov])) +
    ggtitle(bquote(EM: beta[Ecov]==0)) + theme(plot.title = element_text(hjust = 0.5)) + labs(colour = expression(sigma[Ecov]:rho[Ecov])) +
    geom_errorbar(aes(ymin = ymin, ymax = ymax), width = .05, position = position_dodge(0.1))
plt
ggsave(here("Ecov_study","mortality", "paper", "mean_M_bias_OM_EM_PE_match_Ecov_beta_fixed.png"), plt, width = 20, height = 12, units = "in")
remove(all_res)
...
plt <- ggplot(all_res, aes(x = Ecov_effect, y = bias_est, colour = Ecov_re_sig:Ecov_re_cor)) + scale_colour_viridis_d() + 
    ...
    theme_bw() + coord_cartesian(ylim = c(-1, 1)) + ylab(bquote(Median~Bias~of~beta[M])) + xlab(expression(beta[Ecov])) +
    ggtitle(bquote(EM: beta[Ecov]~Estimated)) + theme(plot.title = element_text(hjust = 0.5)) + labs(colour = expression(sigma[Ecov]:rho[Ecov])) +
    geom_errorbar(aes(ymin = ymin, ymax = ymax), width = .05, position = position_dodge(0.1))
plt
ggsave(here("Ecov_study","mortality", "paper", "mean_M_bias_OM_EM_PE_match_Ecov_beta_estimated.png"), plt, width = 20, height = 12, units = "in")
remove(all_res)
...
plt <- ggplot(all_res, aes(x = Ecov_effect, y = bias_est, colour = re_config)) + scale_colour_viridis_d() + 
    ...
    theme_bw() + coord_cartesian(ylim = c(-0.5, 0.5)) + ylab(bquote(Median~Error~of~hat(beta)[M])) + xlab(expression(beta[M])) +
    ggtitle(bquote(EM: beta[Ecov]==0)) + theme(plot.title = element_text(hjust = 0.5)) + labs(colour = "EM process error") +
    geom_errorbar(aes(ymin = ymin, ymax = ymax), width = .05, position = position_dodge(0.1))
plt
ggsave(here("Ecov_study","mortality", "paper", "mean_M_bias_all_PE_effect_Ecov_beta_fixed.png"), plt, width = 20, height = 12, units = "in")
remove(all_res)
...
plt <- ggplot(all_res, aes(x = Ecov_effect, y = bias_est, colour = re_config)) + scale_colour_viridis_d() + 
    ...
    facet_grid(Ecov_obs_sig+Ecov_re_sig+Ecov_re_cor  ~ NAA_M_re + Fhist + obs_error, 
      labeller = labeller(Ecov_re_sig = label_parsed, Ecov_re_cor = label_parsed, obs_error = label_wrap_gen(width = 15), Ecov_obs_sig = label_parsed, Fhist = label_parsed)) +
    theme_bw() + coord_cartesian(ylim = c(-0.5, 0.5)) + ylab(bquote(Median~Error~of~hat(beta)[M])) + xlab(expression(beta[M])) +
    ggtitle(bquote(EM: beta[Ecov]~Estimated)) + theme(plot.title = element_text(hjust = 0.5)) + labs(colour = "EM process error") +
    geom_errorbar(aes(ymin = ymin, ymax = ymax), width = .05, position = position_dodge(0.1))
plt
ggsave(here("Ecov_study","mortality", "paper", "mean_M_bias_all_PE_effect_Ecov_beta_estimated.png"), plt, width = 20, height = 12, units = "in")
remove(all_res)
