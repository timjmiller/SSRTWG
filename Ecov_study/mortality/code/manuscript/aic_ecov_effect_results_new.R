...
plt <- ggplot(temp, aes(x = Ecov_effect, y = n, fill = correct)) + scale_fill_viridis_d(begin = 0.2, end = 0.8, option = "turbo", drop = FALSE) + 
    ...
    facet_nested(Ecov_obs_sig+Ecov_re_sig+Ecov_re_cor ~ OM_process_error + Fhist, 
      labeller = labeller(Ecov_obs_sig = label_parsed, Ecov_re_sig = label_parsed, Ecov_re_cor = label_parsed, Fhist = label_parsed)) +
    coord_cartesian(ylim = c(0, 1)) + ylab("Probability lowest AIC") + xlab(expression("True "*beta[Ecov]))
plt
cairo_pdf(here("Ecov_study","mortality","manuscript", "aic.pdf"), width = 30*2/3, height = 20*2/3)
print(plt)
dev.off()

# temp <- subset(all_res_mod, obs_error == "Low observation error" & !M_est)
plt <- ggplot(temp, aes(x = Ecov_effect, y = n, fill = EM_process_error)) + scale_fill_viridis_d(begin = 0.2, end = 0.8, option = "turbo", drop = FALSE) + 
    geom_col(position = "fill") + labs(fill = "EM PE Assumption") + 
    facet_nested(Ecov_obs_sig+Ecov_re_sig+Ecov_re_cor ~ OM_process_error + Fhist, 
      labeller = labeller(Ecov_obs_sig = label_parsed, Ecov_re_sig = label_parsed, Ecov_re_cor = label_parsed, Fhist = label_parsed)) +
    coord_cartesian(ylim = c(0, 1)) + ylab("Probability lowest AIC") + xlab(expression("True "*beta[Ecov]))
plt
cairo_pdf(here("Ecov_study","mortality","manuscript", "aic_supp1.pdf"), width = 30*2/3, height = 20*2/3)
print(plt)
dev.off()


temp <- subset(all_res_mod, obs_error == "Low observation error" & Fhist != "2.5*italic(F)[MSY] %->% italic(F)[MSY]")
plt <- ggplot(temp, aes(x = Ecov_effect, y = n, fill = correct)) + scale_fill_viridis_d(begin = 0.2, end = 0.8, option = "turbo", drop = FALSE) + 
    geom_col(position = "fill") + labs(fill = "EM assumption") + 
    facet_nested(Ecov_obs_sig+Ecov_re_sig+Ecov_re_cor ~ OM_process_error + M_est, 
      labeller = labeller(Ecov_obs_sig = label_parsed, Ecov_re_sig = label_parsed, Ecov_re_cor = label_parsed)) +
    coord_cartesian(ylim = c(0, 1)) + ylab("Probability lowest AIC") + xlab(expression("True "*beta[Ecov]))
plt
cairo_pdf(here("Ecov_study","mortality","manuscript", "aic_supp2.pdf"), width = 30*2/3, height = 20*2/3)
print(plt)
dev.off()

temp <- subset(all_res_mod, obs_error == "High observation error" & !M_est)
plt <- ggplot(temp, aes(x = Ecov_effect, y = n, fill = correct)) + scale_fill_viridis_d(begin = 0.2, end = 0.8, option = "turbo", drop = FALSE) + 
    geom_col(position = "fill") + labs(fill = "EM assumption") + 
    facet_nested(Ecov_obs_sig+Ecov_re_sig+Ecov_re_cor ~ OM_process_error + Fhist, 
      labeller = labeller(Ecov_obs_sig = label_parsed, Ecov_re_sig = label_parsed, Ecov_re_cor = label_parsed, Fhist = label_parsed)) +
    coord_cartesian(ylim = c(0, 1)) + ylab("Probability lowest AIC") + xlab(expression("True "*beta[Ecov]))
plt
cairo_pdf(here("Ecov_study","mortality","manuscript", "aic_supp3.pdf"), width = 30*2/3, height = 20*2/3)
print(plt)
dev.off()
