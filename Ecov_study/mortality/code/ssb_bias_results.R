...
plt <- ggplot(temp, aes(x = year, y = bias_est, colour = Ecov_obs_sig:Ecov_re_sig:Ecov_re_cor)) + scale_colour_viridis_d() + 
    ...
    ylab(bquote(Median~Relative~Error*"(SSB)")) + xlab("Time") + 
    ggtitle(bquote(beta[M] == log(0.2))) +
...
plt
ggsave(here("Ecov_study","mortality", "paper", "SSB_bias_all_PE_effect_M_fixed.png"), plt, width = 20, height = 12, units = "in")
remove(all_res)
...
plt <- ggplot(temp, aes(x = year, y = bias_est, colour = Ecov_obs_sig:Ecov_re_sig:Ecov_re_cor)) + scale_colour_viridis_d() + 
    ...
    theme_bw() +
    coord_cartesian(ylim = c(-0.25, 0.5)) + 
    ylab(bquote(Median~Relative~Error*"(SSB)")) + xlab("Time") + ggtitle(bquote(beta[M]~Estimated)) +
    theme(plot.title = element_text(hjust = 0.5), panel.spacing = unit(0.01, "lines")) + labs(colour = bquote(sigma[ecov]:sigma[Ecov]:rho[Ecov])) +
    geom_errorbar(aes(ymin = ymin, ymax = ymax), width = .5, linewidth = 1, position = position_dodge(pdodge))
plt
ggsave(here("Ecov_study","mortality", "paper", "SSB_bias_all_PE_effect_M_estimated.png"), plt, width = 20, height = 12, units = "in")
remove(all_res)
