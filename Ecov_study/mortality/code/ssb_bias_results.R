...
#    print(om_ind)
    res <- lapply(om_ind, function(x) {
      ssb_res <- t(sapply(1:40, function(y) {
        #print(om_ind)
        r_e <- ssb_bias[[x]][,em_ind,y,2]/ssb_bias[[x]][,em_ind,y,1]-1
        out <- c(median(r_e, na.rm = TRUE), sd(r_e, na.rm=T)/sqrt(sum(!is.na(r_e))), custom_boxplot_stat(r_e))#quantile(x, probs = c(0.025,0.975)))
        return(out)
      }))
      colnames(ssb_res) <- c("bias_est", "bias_se", "ymin", "lower", "middle", "upper", "ymax")
      return(ssb_res)
    })
    # print(res[[1]])
    # print(dim(res[[1]]))
...
temp <- filter(all_res, Fhist == "FMSY" & NAA_M_re == "R+S" & Ecov_re_sig == 0.5 & Ecov_effect == 0.5 & Ecov_re_cor == 0.5)

x <- which(df.oms$Fhist == "MSY" & df.oms$NAA_M_re == "rec+1" & df.oms$Ecov_re_sig == 0.5 & df.oms$Ecov_effect == 0.5 & df.oms$Ecov_re_cor == 0.5)
#temp <- all_res
...
plt <- ggplot(temp, aes(x = Ecov_effect, y = bias_est, colour = year)) + scale_colour_viridis_d(alpha = 0.7) + 
    ...
    ylab(bquote(Median~relative~error~of~SSB)) + xlab(expression(beta[Ecov])) +
...
plt
ggsave(here("Ecov_study","mortality", "paper", "ssb_bias_OM_EM_PE_match_M_fixed.png"), plt, width = 20, height = 12, units = "in")
remove(all_res)
...
plt <- ggplot(temp, aes(x = Ecov_effect, y = bias_est, colour = year)) + scale_colour_viridis_d(alpha = 0.7) + 
    ...
    geom_errorbar(aes(ymin = ymin, ymax = ymax), width = .5, position = position_dodge(0.6), linewidth = 1)
plt
ggsave(here("Ecov_study","mortality", "paper", "ssb_bias_OM_EM_PE_match_M_estimated.png"), plt, width = 20, height = 12, units = "in")
remove(all_res)
...
plot_df_fn <- function(df.ems, df.oms, M_est = FALSE, Ecov_est = FALSE) {
  for(h in 1:3) {
    ...
    #   for(j in em_ind){
    res <- lapply(om_ind, function(x) {
      ssb_res <- lapply(1:40, function(y) {
        #print(om_ind)
        out <- matrix(NA,length(em_ind), 7)
        for(j in em_ind){
          r_e <- ssb_bias[[x]][,j,y,2]/ssb_bias[[x]][,j,y,1]-1
...
all_res <- plot_df_fn(df.ems, df.oms, Ecov_est = FALSE, M_est = FALSE)
temp <- filter(all_res, year %in% c(1,21,40))
temp$Ecov_effect <- factor(temp$Ecov_effect)
temp$year <- factor(temp$year)
temp <- temp %>% mutate(year = recode(year,
...
plt <- ggplot(temp, aes(x = year, y = bias_est, colour = Ecov_obs_sig:Ecov_re_sig:Ecov_re_cor)) + scale_colour_viridis_d() + 
    ...
    theme_bw() + coord_cartesian(ylim = c(-0.3, 0.3)) + 
    ylab(bquote(Median~Relative~Error~of~widehat(SSB))) + xlab("Time") + 
...
plt
ggsave(here("Ecov_study","mortality", "paper", "SSB_bias_all_PE_effect_M_fixed_beta_fixed.png"), plt, width = 20, height = 12, units = "in")
remove(all_res)

all_res <- plot_df_fn(df.ems, df.oms, M_est = TRUE, Ecov_est = FALSE)
temp <- filter(all_res, year %in% c(1,21,40))
temp$Ecov_effect <- factor(temp$Ecov_effect)
temp$year <- factor(temp$year)
temp <- temp %>% mutate(year = recode(year,
...
plt <- ggplot(temp, aes(x = year, y = bias_est, colour = Ecov_obs_sig:Ecov_re_sig:Ecov_re_cor)) + scale_colour_viridis_d() + 
    ...
    geom_errorbar(aes(ymin = ymin, ymax = ymax), width = .01, linewidth = 1, position = position_dodge(pdodge))
plt
ggsave(here("Ecov_study","mortality", "paper", "SSB_bias_all_PE_effect_M_estimated_beta_fixed.png"), plt, width = 20, height = 12, units = "in")
remove(all_res)

all_res <- plot_df_fn(df.ems, df.oms, M_est = FALSE, Ecov_est = TRUE)
temp <- filter(all_res, year %in% c(1,21,40))
temp$Ecov_effect <- factor(temp$Ecov_effect)
temp$year <- factor(temp$year)
temp <- temp %>% mutate(year = recode(year,
...
plt <- ggplot(temp, aes(x = year, y = bias_est, colour = Ecov_obs_sig:Ecov_re_sig:Ecov_re_cor)) + scale_colour_viridis_d() + 
    ...
    geom_errorbar(aes(ymin = ymin, ymax = ymax), width = .01, linewidth = 1, position = position_dodge(pdodge))
plt
ggsave(here("Ecov_study","mortality", "paper", "SSB_bias_all_PE_effect_M_fixed_beta_estimated.png"), plt, width = 20, height = 12, units = "in")
remove(all_res)

all_res <- plot_df_fn(df.ems, df.oms, M_est = TRUE, Ecov_est = TRUE)
temp <- filter(all_res, year %in% c(1,21,40))
temp$Ecov_effect <- factor(temp$Ecov_effect)
temp$year <- factor(temp$year)
temp <- temp %>% mutate(year = recode(year,
...
plt <- ggplot(temp, aes(x = year, y = bias_est, colour = Ecov_obs_sig:Ecov_re_sig:Ecov_re_cor)) + scale_colour_viridis_d() + 
    ...
    geom_errorbar(aes(ymin = ymin, ymax = ymax), width = .01, linewidth = 1, position = position_dodge(pdodge))
plt
ggsave(here("Ecov_study","mortality", "paper", "SSB_bias_all_PE_effect_M_estimated_beta_estimated.png"), plt, width = 20, height = 12, units = "in")
remove(all_res)
