...
em_input <- readRDS(file.path(write.dir, paste0("em_input_",args[1],".RDS")))
...
em_fits <- foreach(x = 1:nsim) %dopar% {
    ...
    out = fit_wham(em_input[[x]], do.osa = FALSE, MakeADFun.silent = TRUE, retro.silent = TRUE, save.sdrep = FALSE)
    out$rep <- out$report()
