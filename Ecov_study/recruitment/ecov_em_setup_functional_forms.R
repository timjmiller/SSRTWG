...
df.ems <- expand.grid(Ecov_how=Ecov_how, M_est = M_est, re_config = re_config, Ecov_est = Ecov_est, stringsAsFactors = FALSE)
saveRDS(df.ems, file.path(here(),"Ecov_study", "recruitment", "inputs_05_10_2023", "df.ems.RDS"))
...
for(i in 1:NROW(df.ems)){
  ...
  if(df.ems$Ecov_est[i]){
    ...
    if(df.ems$Ecov_how[i] == 1) ecov_i$how = 1
    if(df.ems$Ecov_how[i] == 2) ecov_i$how = 2
    if(df.ems$Ecov_how[i] == 4) ecov_i$how = 4
  }
...
saveRDS(em_inputs, file.path(here(),"Ecov_study", "recruitment", "inputs_05_10_2023", "em_inputs.RDS"))
