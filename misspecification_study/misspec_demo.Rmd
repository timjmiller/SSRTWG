...
The process starts with creating an intial data set using the make_basic_info function and assigning values for selectivity, natural mortality, and recruitment. For this example, there is a single fleet and two indices all with logisitic selectivity, natural mortality is constant at 0.2 for all years and ages, there is no stock-recruitment relationship (recruitment varies as an identical and independent deviation about the mean). The prepare_wham_input function creates the correctly specified input formulation. 
...
Simulated data are created under two conditions: correctly an incorrectly reported catch in the recent years of the assessment. The incorrectly reported catch uses the bias_data function. In this example, catch is under-reported by half (the catch provided to the wham model is only half of the actual catch) for the econd half of the assessment time period. The four estimation models are then set up to be applied to the two sets of data, resulting in a total of 8 models. The first four models use the correct catch data while the last four models use the under-reported catch data.
...
set.seed(14159265) #use same seed for all operating models
agg_catch_multiplier <- create_agg_catch_multiplier(input, multiplier=0.50)
...
for (m in 1:8){
  ...
  thisaggc <- tibble(Model = m, Year = 1:sim_input[[m]][[1]]$data$n_years_model, AggCatch = unlist(aggcdat))
...
Start by comparing the Mohn's rho for SSB. The misspecified models all have larger magnitude Mohn's rho for SSB than the models with the correct catch.

```{r rhoSSB}

rhoSSB <- matrix(NA, nrow=8, ncol=2)
colnames(rhoSSB) <- c("Model", "rhoSSB")
for (m in 1:8){
  thisrho <- lapply(1:nsim, function(x){
    out <- mohns_rho(em_fits[[m]][[x]])[1]
  return(out)
  })
  rhoSSB[m,] <- c(m, unlist(thisrho))
}
knitr::kable(rhoSSB)
```

We can see which model has the best fit using the compare_wham_models function for the two sets of data.

```{r compare}
correct_data_compare <- compare_wham_models(list(em_fits[[1]][[1]],em_fits[[2]][[1]],em_fits[[3]][[1]],em_fits[[4]][[1]]))

misspecified_data_compare <- out <- compare_wham_models(list(em_fits[[5]][[1]],em_fits[[6]][[1]],em_fits[[7]][[1]],em_fits[[8]][[1]]))
