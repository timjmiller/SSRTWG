make_basic_info <- function(base_years = 1982:2021, ages = 1:10, Fhist = "updown", n_feedback_years = 0) { #changed years
  ...
  nwaa <- info$n_indices + info$n_fleets + 2  # number of 
  ...
  # make q time-varying?  t(matrix(info$q, info$n_fleets, ny))
...
set_NAA_pars <- function(pars, input)
{
  ...
  if("NAA_rho" %in% names(pars)) input$par$trans_NAA_rho[] = wham:::gen.logit(pars$NAA_rho, -1,1)
