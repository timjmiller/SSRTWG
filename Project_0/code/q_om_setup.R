...
obs_error = c("L", "H")
df.q.oms <- expand.grid(R_sig = R_sig, q_sig = q_sig, q_cor = q_cor, Fhist = Fhist, 
...
n.mods <- dim(df.q.oms)[1] #24 operating model scenarios
df.q.oms$Model <- paste0("om_",1:n.mods)
...
for(i in 1:NROW(df.q.oms)){
  ...
  om_inputs[[i]]$par$catch_paa_pars[,1] = log(L_N_sigma[df.q.oms$obs_error[i]])
  om_inputs[[i]]$par$index_paa_pars[,1] = log(L_N_sigma[df.q.oms$obs_error[i]])
  om_inputs[[i]]$data$agg_catch_sigma[] = 0.1
  om_inputs[[i]]$data$agg_index_sigma[] = index_sigma[df.q.oms$obs_error[i]]
  #change Neff so scalar doesn't affect L-N SD
  om_inputs[[i]]$data$catch_Neff[] = 1
  om_inputs[[i]]$data$index_Neff[] = 1
}

sapply(om_inputs, function(x) x$par$q_repars[,1])
sapply(om_inputs, function(x) x$par$q_repars[,2])
sapply(om_inputs, function(x) x$par$q_repars[1,])
sapply(om_inputs, function(x) x$data$use_q_re)
saveRDS(om_inputs, file.path(here(),"Project_0","inputs", "q_om_inputs.RDS"))
