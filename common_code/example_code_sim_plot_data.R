...
sim.out=here("Ecov_study", "mortality", "simdata6")
...
om.inputs.rds=file.path(here(), "Ecov_study", "recruitment", "inputs", "om_inputs.RDS") #inputs for oms
df.oms.rds=file.path(here(),"Ecov_study", "recruitment", "inputs", "df.oms.RDS")        #matrix of om specs
sim.out=here("Ecov_study", "recruitment", "simdata6")

om.nums <-  seq(197,200)  #c(9,33,393,417)  #seq(9,12)
sim.nums <- c(22,116)  #c(25, 44, 95)
sim_data <- simulate_data(seed.rds=seed.rds,   #pass list of seeds
                          same.seeds=TRUE,  # if TRUE, uses the same 1000 seeds for all oms
                          om.inputs.rds=om.inputs.rds, #pass name of rds file with om.inputs
                          df.oms.rds=df.oms.rds,    #pass name of rds file with df.om specs
