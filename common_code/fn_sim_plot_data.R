...
############# get_axis_scalar fn ====
# copied from: https://stackoverflow.com/questions/3099219/ggplot-with-2-y-axes-on-each-side-and-different-scales

# Function factory for secondary axis transforms
get_axis_scalar <- function(primary, secondary, na.rm = TRUE) {
...
plot_SR <- function(sim_data=NULL, simdata_dir=NULL,
  ...
  Ecov.slot <-  which((names(sim_data[[1]][[1]])== "Ecov_obs" ))
  ...
  Ecov.obj <- lapply(sim_data, function(x) lapply(x, function(y)  y[Ecov.slot] )   ) 
  Ecov.tib <-  as_tibble(lapply(Ecov.obj, function(x) as_tibble(unlist(x) )) )
  ...
  len.Ecov <- dim(as.data.frame(Ecov.obj[[1]][1]) ) [1]
  years.Ecov <- seq(1, len.Ecov)
  ...
  Rec.tib1a <- Rec.tib %>%
   ...
   Rec.mean.tib <- Rec.tib1a %>%
     group_by(sim, OM) %>%
     dplyr::summarise(Rmean=mean(Recruits))
   
   Rec.tib1 <- Rec.tib1a %>%
     left_join(Rec.mean.tib)
 
  Ecov.tib1 <- Ecov.tib %>%
    mutate(sim = rep(rep(sim.nums, each=len.Ecov)))  %>%
    mutate( Year = rep(years.Ecov, n_sim)) %>%
    pivot_longer(cols=starts_with("om"), names_to="OM", values_to="Value")  %>%
    mutate( OM=as.factor(OM))  %>%
    mutate(Ecov = unlist(Value))  %>%
    select(-Value)
  ...
  Rec.tib.all.yrs <- Rec.tib %>%
   ...
   SR.tib <- SSB.tib1 %>%
     left_join(Rec.tib1) %>%
     left_join(Ecov.tib1)  %>%
     mutate(RperS = Recruits/SSB)  %>%
     mutate(Ecov_obs = exp(Ecov))  %>%
     mutate(Recr.resid = (Recruits-Rmean) )  %>%
     mutate(RperS.resid = (RperS - (Rmean/SSB)))
  ...
  # range.ecov <- range(SR.tib$Ecov_obs, na.rm=T)
  # range.RperS <- range(SR.tib$RperS, na.rm=T)
  # range.scale <- min(abs(range.ecov)/abs(range.RperS))
  # range.scale <- max(abs(range.ecov))/mean(SR.tib$RperS, na.rm=T)
  
  sec <- get_axis_scalar(SR.tib$RperS, SR.tib$Ecov_obs)
  
  
   RperS.plot <- ggplot( SR.tib, aes(x=Year))  +
     facet_wrap(~OM) +
     geom_line(aes( y=RperS , color=as.factor(sim))  ) +
     geom_point( aes( y=sec$fwd(Ecov_obs), color=as.factor(sim) ) ) +
     scale_y_continuous(
       
       # Features of the first axis
       name = "Recruits per Spawner",
       
       # Add a second axis and specify its features
       #sec.axis = sec_axis(~.*range.scale, name="Ecov_obs")
       sec.axis = sec_axis(~sec$rev(.), name = "Ecov_obs")
     ) + 
     ...
     theme(
       ...
       axis.title.y.right = element_text(color = 'grey45', size=13)
     ) +
     theme(
       strip.text.x = element_text(
         size = 12, color = "black"),
   ...
   ggsave(RperS.plot, filename = file.path(simdata_dir, paste0("om_", prefix.plot, "_RperS.year.plot",  ".png")),
          height=pheight, width=pwidth)
   ...
   # range.ecov <- range(SR.tib$Ecov_obs, na.rm=T)
   # range.Recr.resid <- range(SR.tib$Recr.resid, na.rm=T)
   # range.scale2 <- max(abs(range.ecov)/abs(range.Recr.resid))
   sec2 <- get_axis_scalar(SR.tib$Recr.resid, SR.tib$Ecov_obs)
   
   
   Recr.resid.plot <- ggplot( SR.tib, aes(x=Year, y=Recr.resid , color=as.factor(sim)))  +
     facet_wrap(~OM) +
     geom_line() +
     geom_point(data=SR.tib, aes(x=Year, y=sec2$fwd(Ecov_obs), color=as.factor(sim)), ) +
     scale_y_continuous(
       
       # Features of the first axis
       name = "Recruits - Mean(Recruits)",
       
       # Add a second axis and specify its features
       sec.axis = sec_axis(~sec2$rev(.), name = "Ecov_obs")
   ...
   ggsave(Recr.resid.plot, filename = file.path(simdata_dir, paste0("om_", prefix.plot, "_Recr.resid.yr.plot",  ".png")),
          height=pheight, width=pwidth)
   
   
   Recr.resid.Ecov.plot <- ggplot( SR.tib, aes(x=Ecov_obs, y=Recr.resid , color=as.factor(sim)))  +
     facet_wrap(~OM) +
     geom_point() +
          theme_light()  +
   ...
   ggsave(Recr.resid.Ecov.plot, filename = file.path(simdata_dir, paste0("om_", prefix.plot, "_Recr.resid.Ecov.plot",  ".png")),
          height=pheight, width=pwidth)
   

   
   RperS.Ecov.plot <- ggplot( SR.tib, aes(x=Ecov_obs, y=RperS , color=as.factor(sim)))  +
     facet_wrap(~OM) +
     geom_point() +
     theme_light()  +
   ...
   ggsave(RperS.Ecov.plot, filename = file.path(simdata_dir, paste0("om_", prefix.plot, "_RperS.Ecov.plot.plot",  ".png")),
          height=pheight, width=pwidth)
   

   RperS.resid.Ecov.plot <- ggplot( SR.tib, aes(x=Ecov_obs, y=RperS.resid , color=as.factor(sim)))  +
     facet_wrap(~OM) +
     geom_point() +
     theme_light()  +
   ...
   ggsave(RperS.resid.Ecov.plot, filename = file.path(simdata_dir, paste0("om_", prefix.plot, "_RperS.resid.Ecov.plot",  ".png")),
          height=pheight, width=pwidth)
